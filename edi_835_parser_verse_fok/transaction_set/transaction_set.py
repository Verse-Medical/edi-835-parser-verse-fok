...
from edi_835_parser_verse_fok.loops.claim import Claim as ClaimLoop
from edi_835_parser_verse_fok.loops.service import Service as ServiceLoop
from edi_835_parser_verse_fok.loops.organization import Organization as OrganizationLoop
from edi_835_parser_verse_fok.segments.utilities import find_identifier
from edi_835_parser_verse_fok.segments.interchange import Interchange as InterchangeSegment
from edi_835_parser_verse_fok.segments.financial_information import FinancialInformation as FinancialInformationSegment

BuildAttributeResponse = namedtuple('BuildAttributeResponse', 'key value segment segments')

class IteratorWithCounter:
...
class TransactionSet:
	...
	def build(cls, file_path: str) -> 'TransactionSet':
		...
		segments = IteratorWithCounter(iter(segments))
		segment = None

		claim_indices = []

		while True:
			start_index = segments.counter
			response = cls.build_attribute(segment, segments)
			if response.segments is not None:
				end_index = response.segments.counter
			else:
				end_index = None	
			...
			if response.key == 'claim':
				...
				claim_indices.append((start_index, end_index))

		ts = TransactionSet(interchange, financial_information, claims, organizations, file_path)
		ts.original_segments = original_segments
		ts.claim_indices = claim_indices
